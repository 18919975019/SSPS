...
def compute_vote_loss(end_points, supervised_inds):
    ...
    vote_xyz = end_points['vote_xyz'][supervised_inds,
                                      ...]  # B_l,num_seed*vote_factor,3
    seed_inds = end_points['seed_inds'][
        supervised_inds, ...].long()  # B_l,num_seed in [0,num_points-1]
    ...
    seed_inds_expand = seed_inds.view(batch_size_supervised, num_seed,
                                      1).expand(-1, -1, 3 * GT_VOTE_FACTOR)
    seed_gt_votes = torch.gather(vote_label, 1,
                                 seed_inds_expand)  # B_l,num_seed,9
    seed_gt_votes += seed_xyz.repeat(1, 1, 3)

    vote_xyz_reshape = vote_xyz.view(
        batch_size_supervised * num_seed, -1,
        3)  # from B_l,num_seed*vote_factor,3 to B_l*num_seed,vote_factor,3
    seed_gt_votes_reshape = seed_gt_votes.view(
        batch_size_supervised * num_seed, GT_VOTE_FACTOR, 3
    )  # from B_l,num_seed,3*GT_VOTE_FACTOR to B_l*num_seed,GT_VOTE_FACTOR,3
    # A predicted vote to no where is not penalized as long as there is a good vote near the GT vote.
    dist1, _, dist2, _ = nn_distance(vote_xyz_reshape,
                                     seed_gt_votes_reshape,
                                     l1=True)
    votes_dist, _ = torch.min(
        dist2, dim=1)  # (B_l*num_seed,vote_factor) to (B_l*num_seed,)
...
def compute_objectness_loss(end_points, supervised_inds):
    ...
    gt_mask = (1 - end_points['box_label_mask'][supervised_inds,
                                                ...]).unsqueeze(-1).expand(
                                                    -1, -1, 3).bool()
    gt_center[gt_mask] = -1000

    dist1, ind1, dist2, _ = nn_distance(
        aggregated_vote_xyz, gt_center)  # dist1: B_l xK, dist2: B_l xK2
...
def calculate_segmentation_loss(output, target, mask_type):
    ...
    criterion = SetCriterion(num_classes=20, 
                             matcher=matcher,
